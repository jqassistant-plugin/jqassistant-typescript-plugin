...
export class CodeCoordinateUtils {
    static getCodeCoordinates(globalContext: GlobalContext, node: Node, saveFilePath: boolean = false): CodeCoordinates {
        const sourceFile = globalContext.services.program.getSourceFile(
            path.resolve(globalContext.projectRootPath, globalContext.sourceFilePath)
        );
        ...
        return new CodeCoordinates(
            saveFilePath ? globalContext.sourceFilePath : undefined,
